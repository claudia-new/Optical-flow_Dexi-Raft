...
class DexiNed(nn.Module):
    ...
    def forward(self, x):
        ...
        if not out_5.shape==out_1.shape:
            h, w = out_1.shape[-2], out_1.shape[-1]            
            h_max, w_max = out_5.shape[-2], out_5.shape[-1]
            assert (h_max >= h and w_max >= w)
            h_offset, w_offset = h_max - h, w_max - w
            out_5 = out_5[:, :, h_offset:h_offset+h, w_offset:w_offset+w]
            out_6 = out_6[:, :, h_offset:h_offset+h, w_offset:w_offset+w]            
