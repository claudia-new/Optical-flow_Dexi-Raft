...
class RAFT(nn.Module):
    ...
    def forward(self, image1, image2, iters=12, flow_init=None, upsample=True, test_mode=False):
        ...
        with autocast(enabled=self.args.mixed_precision):
            fmap1, fmap2 = self.fnet([imedge1, imedge2])                
        
        '''
        edge1 = torch.stack(edge1)
        em1 = torch.squeeze(edge1)
        em1 = em1.permute(1, 0, 2, 3)
        edge2 = torch.stack(edge2)
        em2 = torch.squeeze(edge2)
        em2 = em2.permute(1, 0, 2, 3)
        #down_em1 = self.maxpool(self.maxpool(self.maxpool(em1)))
        
        print(' the type of down_em1: ', type(em1), down_em1.shape)
        print(' the type and size of fmap is: ', type(fmap1), fmap1.shape)
        #em1 = torch.Tensor(em1)
        #em2 = em2.float()
        '''
        fmap1 = fmap1.float()
        fmap2 = fmap2.float()
